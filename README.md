...
> 프로젝트의 원활한 진행을 위한 팀원별 역할 및 주요 업무 분담 내역입니다.

### 변슬기
**담당 파트: 인증 및 온보딩 (Auth & Onboarding)**

**주요 업무**
- Splash 화면 구현
- 로그인 (메인, 이메일)
- 회원가입 및 프로필 설정
- 싱글 페이지 웹(SPA) 라우팅 초기 세팅 보조

**핵심 구현**
- 이메일, 비밀번호 등 엄격한 실시간 유효성 검사 로직 구현

### 박미소
**담당 파트: 메인 피드 및 공통 UI (Feed & Navigation)**

**주요 업무**
- 감귤마켓 피드 (홈 화면) UI 구현
- 하단 탭 메뉴 (페이지별 활성화 상태 관리)
- 좋아요 버튼 UI 토글 상태 변경 및 개수 카운트 기능
- 전체 앱 상태 관리 초기 셋업
...
### 손은애
**담당 파트: 프로필 및 사용자 관리 (Profile & User)**

**주요 업무**
- 사용자 프로필 페이지 (내 프로필 / 타인 프로필 분기 처리)
- 팔로워, 팔로잉 목록 페이지 및 동적 업데이트
- 내 프로필 수정 페이지
- 팔로우 / 언팔로우 토글 로직 구현

**핵심 구현**
- 데이터에 따른 상품 및 게시글 조건부 렌더링

### 백동명
**담당 파트: 콘텐츠 생성 및 AI (Creation & AI)**

**주요 업무**
- 게시글 작성 페이지 (이미지 1장 업로드)
- 상품 등록 페이지 (가격 자동 원 단위 변환 포맷팅)
- 다중 이미지 업로드 (최대 3장 제한 로직 구현)

**핵심 구현**
- AI 활용 기능  
  (상품 설명 자동 생성 API 연동, 스트리밍 응답, 덮어쓰기 모달 구현)

### 강민기
**담당 파트: 상호작용 및 공통 모달 (Interaction & Modals)**

**주요 업무**
- 게시글 댓글 페이지 (`postDetail.jsx`) 댓글 확인 및 입력 UI
- 공통 모달 UI (헤더, 게시글, 댓글용)
- 댓글 삭제 및 신고하기 로직
...
### 전체
**담당 파트: 탐색 및 채팅 UI (Search & Chat UI)**

**주요 업무**
- 채팅 목록 페이지 (마크업 및 안 읽은 표시 UI)
- 채팅방 페이지 (마크업 및 전송 버튼 활성화 로직)
- 검색 페이지 마크업 UI
- 검색 로직 구현 (이름 / 계정 검색)
- 일치 단어 하이라이트 처리

**핵심 구현**
- 요구사항 명세 기반 UI 및 기능 반영
